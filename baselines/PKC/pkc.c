 ...
 * Description: A parallel k-core decomposition algorithm for multicore
 * processor. It is designed for large sparse graphs.
 *
 * Please cite the following paper, if you are using PKC:
 *     H. Kabir and K. Madduri, "Parallel k-core Decomposition on Multicore
 * Platforms," in The 2nd IEEE Workshop on Parallel and Distributed Processing
 * for Computational Social Systems (ParSocial 2017), June 2017, to appear.
 * ********************************************************************************************/
...
// in PKC and PKC_org
#define BUFF_MULT_FACTOR 100

// This can be set to 0 once you make sure there's adequate memory for your
// use-case
...
int free_graph(graph_t *g) {
  if (g->num_edges != NULL) free(g->num_edges);
...
int load_graph_from_binary(char *filename, graph_t *g) {
  ...
  g->n = ((uint64_t *)data)[0];
  g->m = ((uint64_t *)
              data)[1];  // This is the total count of edges in both directions
  size_t sizes = ((uint64_t *)data)[2];
  printf("n: %lu, m: %lu, sizes: %lu\n", g->n, g->m, sizes);
  // assert(sizes == (g->n + 1) * 8 + g->m * 8 + 3 * 8);

  // Allocate space for num_edges and initialize it to zero
  ...
  // Calculate the cumulative sum for temp_num_edges
  ...
  // // Print debug information for temp_num_edges
  // for (long i = 0; i < g->n + 1; i++) {
  //   printf("temp_num_edges[%ld] = %lu\n", i, temp_num_edges[i]);
  // }
  ...
  // Initialize current position to fill the adjacency list
  ...
  vid_t *edges =
      (vid_t *)(data + 3 * sizeof(uint64_t) + (g->n + 1) * sizeof(eid_t));
  ...
  printf("Adjacency lists filled\n");

  // Print adjacency list for debugging
  for (long i = 0; i < g->n; i++) {
    printf("Vertex %ld: ", i);
    for (eid_t j = temp_num_edges[i]; j < temp_num_edges[i + 1]; j++) {
      printf("%lu ", g->adj[j]);
    }
    printf("\n");
  }
  ...
  for (long i = 0; i < g->n; i++) {
    qsort(g->adj + temp_num_edges[i], g->num_edges[i], sizeof(vid_t),
          vid_compare);
  }
  printf("Done sorting\n");
...
#pragma omp for schedule(static)
  ...
  for (long i = 0; i < g->n; i++) {
    qsort(g->adj + g->num_edges[i], g->num_edges[i + 1] - g->num_edges[i],
          sizeof(vid_t), vid_compare);
  }
...
void serial_processSubLevel(graph_t *g, vid_t *curr, long currTail, int *deg,
                            int level, vid_t *next, long *nextTail) {
  (*nextTail) = 0;
  ...
  while (visited < n) {
    ...
    if (useSmallQ == 0) {
      ...
      while (start < end) {
        ...
        for (eid_t j = g_small.num_edges[v]; j < g_small.num_edges[v + 1];
             j++) {
...
void scan(long n, int *deg, int level, vid_t *curr, long *currTailPtr) {
  // Size of cache line
...
#pragma omp for schedule(static)
  ...
  if (index > 0) {
    ...
    for (long j = 0; j < index; j++) curr[tempIdx + j] = buff[j];
  }

#pragma omp barrier
}

void processSubLevel(graph_t *g, vid_t *curr, long currTail, int *deg,
                     int level, vid_t *next, long *nextTailPtr) {
  // Size of cache line
  const long BUFFER_SIZE_BYTES = 2048;
  const long BUFFER_SIZE = BUFFER_SIZE_BYTES / sizeof(vid_t);
...
void ParK(graph_t *g, int *deg) {
  ...
  vid_t *curr = (vid_t *)malloc(1.1 * n * sizeof(vid_t));
  assert(curr != NULL);

  vid_t *next = (vid_t *)malloc(1.1 * n * sizeof(vid_t));
  assert(next != NULL);
...
#pragma omp for schedule(static)
    ...
    while (todo > 0) {
      ...
      //   printf("tid: %d, level: %d, currTail: %ld\n", tid, level, currTail);
      //   printf("todo: %ld\n", todo);
      while (currTail > 0) {
        todo = todo - currTail;
    ...
    while (visited < n) {
      ...
      if ((useSmallQ == 0) && (visited >= (long)(n * frac))) {
        ...
        if (tid == 0) {
          ...
          newDeg = (int *)malloc(reduceN * sizeof(int));
          mapIndexToVtx =
              (unsigned int *)malloc(reduceN * sizeof(unsigned int));
          g_small.n = reduceN;
          g_small.num_edges = (vid_t *)malloc((reduceN + 1) * sizeof(vid_t));
          g_small.num_edges[0] = 0;
...
#pragma omp barrier
        ...
        if (tid == (NUM_THREADS - 1)) {
          ...
          for (long i = ((tid + 1) * part - 1); i >= (tid * part + 1);
               i--) {  // part can't be unsigned int here
            g_small.num_edges[i] = g_small.num_edges[i - 1];
          }
          g_small.num_edges[tid * part] = cumNumEdges[tid];
        }
...
int main(int argc, char *argv[]) {
  ...
  // load_graph_from_binary(argv[1], &g);
  long n = g.n;
  printf("n: %ld, m: %ld\n", n, g.m);
  /* Contains the core number for each vertex */
  int *core = (int *)malloc(2 * n * sizeof(int));
  assert(core != NULL);
  ...
  // start_time = timer();
  // BZ_kCores(&g, core);
  // double bz_time = timer() - start_time;
  // fprintf(stderr, "BZ time: %9.3lf sec\n", bz_time);
  // fprintf(stderr, "done.\n");
  // printf("BZ total time: %9.3lf sec\n", bz_time);

  // start_time = timer();
  // serial_ParK(&g, core);
  // double park_time = timer() - start_time;
  ...
  fprintf(stderr, "done.\n");
  printf("ParK total time: %9.3lf sec\n", park_time);
  ...
  printf("PKC start \n");
  PKC_org(&g, core);
  double pkc_time = timer() - start_time;
  fprintf(stderr, "PKC-org time: %9.3lf sec\n", pkc_time);
  fprintf(stderr, "done.\n");
  printf("PKC org time: %9.3lf sec\n", pkc_time);

  // Output CSV format for easy parsing (only ParK and PKC)
  printf("CSV_OUTPUT: %s,ParK:%.6f,PKC:%.6f\n", argv[1], park_time, pkc_time);
  /* If the graph has atlest 1000 vertices use PKC  */
  // double time_2 = 0.0;
  // if (n > 1000) {
  //   start_time = timer();
  //   PKC(&g, core);
  //   time_2 = timer() - start_time;
